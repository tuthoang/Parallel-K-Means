...
int main()
{
  ...
  double ***per_iter_centroids = new double**[iterations];
  double *minWCSSArray = new double[iterations];


  for(int i = 0;i < iterations;i++){
    per_iter_centroids[i] = new double*[k];
    minWCSSArray[i] =0;
    for(int j = 0; j < k ; j++){
      per_iter_centroids[i][j] = new double[num_features];
      for(int c = 0; c < num_features; c++){
        per_iter_centroids[i][j][c] = 0;
      }
    }
  }
  ...
  //This big for loop is sort of independent we can just copy all of the clusters into an array and see which ones have the min wcss and then select the one with the min
    ...
    #pragma acc data copyin(x [0:num_samples] [0:num_features], \
                          ...
                          mins [0:num_features],distances[0:num_samples][0:k], min_WCSS,minWCSSArray[0:iterations], per_iter_centroids[0:iterations][0:k][0:num_features]),\
    ...
    for (int loop1 = 0; loop1 < iterations; loop1++)
    {
      ...
      //#pragma acc parallel loop
        for (int i = 0; i < k; i++)
        {
            ...
            //  printf("%d \n",counter);
              ...
              centroids[i][0] = (random[counter][0] + mins[0])%maxes[0];
              centroids[i][1] = (random[counter][1] + mins[1])%maxes[1];
       ...
       //clock_t tStartb = clock();
        ...
        #pragma acc loop private(thresh_met_counter, min_WCSS,minIndex,count) 
        while((thresh_met_counter < (k * num_features) && count < 1000) )
        // while(count<1000)
        {
          //printf("count: %d\n", count);
            thresh_met_counter = 0;
      ...
      for(int i = 0; i < num_samples; i++){
          ...
          for(int c = 0; c < k; c++){
            if (counts[c] == 0)
            {
              counts[c] = 1;
                ...
                centroids[c][0] = (random[c][0] + mins[0]) % maxes[0];
                centroids[c][1] = (random[c][1] + mins[1]) % maxes[1]; // If no data points in group, then reinitialize
        ...
        for(int i = 0; i < k; i++){
          for(int j = 0; j < num_features; j++){
            per_iter_centroids[loop1][i][j] = min_centroids[i][j];
          }
        }
        minWCSSArray[loop1] = wcss;
    }
  }
      // std::cout << "end" << endl;
  int index;
  double minima = INT_MAX;
  for(int l = 0; l < iterations; l++){
    if(minima > minWCSSArray[l]){
      index =l;
      minima = minWCSSArray[l];
    }
  }
  for(int i = 0; i < num_samples; i++){
      for(int j = 0; j < num_features; j++){
        min_centroids[i][j] = per_iter_centroids[index][i][j];
    }
  }
