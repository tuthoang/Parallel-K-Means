...
int main()
{
  ...
  int num_samples = 7500; // total number of data points
    ...
    // centroids = initial_centroids;
      ...
      for(int c = 0; c < k; c++){
        for(int i = 0; i < num_samples; i++){
          if(clusters[i] == c)
            for(int j = 0; j < num_features; j++){
              wcss = wcss + (x[i][j] - centroids[c][j]) * (x[i][j] - centroids[c][j]);
            }
        }
      }
      cout <<wcss<<endl;
